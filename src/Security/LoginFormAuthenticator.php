...
class LoginFormAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider): ?Utilisateur
    {
        ...
        $user = $this->entityManager->getRepository(Utilisateur::class)->findOneBy(['mail' => $credentials['mail']]);
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey): ?Response
    {
        ...
        switch ($token->getUser()->getRoles()[0]) {
            case 'ROLE_ELEVE':
                return new RedirectResponse($this->urlGenerator->generate('eleve.index'));
                break;
            case 'ROLE_MONITEUR':
                return new RedirectResponse($this->urlGenerator->generate('moniteur.index'));
                break;
            case 'ROLE_SECRETAIRE':
                return new RedirectResponse($this->urlGenerator->generate('secretaire.index'));
                break;
            case 'ROLE_GERANT':
                return new RedirectResponse($this->urlGenerator->generate('gerant.index'));
                break;
            case 'ROLE_ADMIN':
